...
// ✅ rename cho đúng nghĩa
export const addVoucher = (voucher) => api.post(REST_API_URL, voucher);

// ✅ Controller BE hiện tại đang POST /{id} để update => giữ POST cho khớp
export const getVoucher = (voucherId) => api.get(`${REST_API_URL}/${voucherId}`);
export const updateVoucher = (voucherId, voucher) =>
  api.put(`${REST_API_URL}/${voucherId}`, voucher);

// Nếu bạn đổi controller sang PUT thì dùng dòng này:
// export const updateVoucher = (voucherId, voucher) => api.put(`${REST_API_URL}/${voucherId}`, voucher);

export const deleteVoucher = (voucherId) => api.delete(`${REST_API_URL}/${voucherId}`);
export const searchVoucher = (keyword) =>
  api.get(`${REST_API_URL}/search`, { params: { p: keyword } });
export const filterVouchers = (params) => api.get(`${REST_API_URL}/filter`, { params });
export const getPagedVouchers = (page, size) =>
  api.get(`${REST_API_URL}/phan-trang`, { params: { page, size } });
export const checkMaTrung = (ma) => api.get(`${REST_API_URL}/check-ma`, { params: { ma } });
export const deactivateVoucher = (voucherId) =>
  api.put(`${REST_API_URL}/ngung-hoat-dong/${voucherId}`);
