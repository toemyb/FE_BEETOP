  ...
  const handleSubmit = () => {
    ...
    if (!combinations.length) {
      return message.warning(
        "Không có tổ hợp nào để tạo. Hãy kiểm tra lại lựa chọn cấu hình."
      );
    }
    ...
    Modal.confirm({
      title: "Xác nhận tự sinh phiên bản",
      content: `Bạn sẽ tạo tối đa ${totalCombos} biến thể cho laptop này. 
Các biến thể đã tồn tại sẽ được bỏ qua. Bạn chắc chắn muốn tiếp tục?`,
      okText: "Đồng ý",
      cancelText: "Hủy",
      onOk: async () => {
        const payload = {
        ...
        try {
          setSubmitting(true);
          const res = await autoGenerateLaptopCT(payload);

          const list = res?.data?.data || res?.data || [];
          const created = Array.isArray(list) ? list.length : 0;
          const duplicated = totalCombos - created;

          if (created > 0) {
            if (duplicated > 0) {
              message.success(
                `Tạo mới ${created} biến thể. ${duplicated} biến thể đã tồn tại nên không tạo thêm.`
              );
            } else {
              message.success(`Tạo mới ${created} biến thể thành công.`);
            }
            navigate(`/admin/lap-top-ct/${idLaptop}`);
          } else {
            // 👉 tất cả tổ hợp đã tồn tại
            message.warning(
              "Tất cả các biến thể với cấu hình này đã tồn tại, không tạo thêm bản nào."
            );
          }
        } catch (e) {
