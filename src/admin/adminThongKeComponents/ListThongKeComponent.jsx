...
const ListThongKeComponent = () => {
  ...
  const fetchThongKe = async () => {
    try {
      ...
      const [tongQuanResponse, trangThaiResponse] = await Promise.all([
        getThongKeTongQuan(params),
        getThongKeTrangThai()
      ]);

      // Merge data từ 2 API
      const mergedData = {
        ...(tongQuanResponse.data?.data || {}),
        ...(trangThaiResponse.data?.data || {})
      };

      setThongKeData(mergedData);
    } catch (error) {
  ...
  // Fetch dữ liệu truy cập (online/offline users)
  const fetchAccessData = async () => {
    try {
      const response = await getThongKeTruyCap();
  ...
  return (
    <div>
      ...
      {/* General Statistics Cards (Moved from top) */}
      <h3 style={{ marginBottom: 16, fontWeight: 600, fontSize: 18 }}>Tổng Quan Doanh Thu</h3>
        ...
        {(() => {
          ...
          const mergedData = Array.from(allProductIds).map(idLaptop => {
            ...
            return {
              ...
              hinhAnh: laptop2?.hinhAnh || laptop1?.hinhAnh || null,
          ...
          return (
            <Table
              ...
              rowKey={(record) => record.idLaptop || record.stt}
              columns={[
                {
                  title: 'STT',
                  dataIndex: 'stt',
                ...
                //     <img
                //       src={hinhAnh || '/placeholder-laptop.png'}
                ...
                //       }}
                //       onError={(e) => {
                //         e.target.src = '/placeholder-laptop.png';
                //       }}
                //     />
                //   )
                // },
