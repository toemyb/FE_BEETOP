...
import { Table, Input, Button, Select, Tag, DatePicker, Space, Pagination, Modal } from 'antd';
...
const ListPhieuGiamGiaComponent = () => {
  ...
  const formatGiaTriMax = (giaTriMax, kieu) => {
    if (kieu !== 'GIAM_PHAN_TRAM') return '-';
    return giaTriMax ? formatCurrency(giaTriMax) : '-';
  };
  ...
  const goEdit = (id) => navigator(`/admin/edit-phieu-giam-gia/${id}`);
  ...
  const columns = [
    ...
    { title: 'Giảm tối đa', render: (_, record) => formatGiaTriMax(record.giaTriMax, record.kieuGiamGia) },
    ...
    { title: 'Trạng thái', render: (_, record) => statusMap[record.trangThai] || <Tag>Không rõ</Tag> },
    {
      ...
      render: (_, record) => (
        <Space size="middle">
          <EditOutlined style={{ color: 'orange', cursor: 'pointer' }} onClick={() => goEdit(record.idPhieugiamgia)} />
          <StopOutlined
            style={{
              color: 'red',
